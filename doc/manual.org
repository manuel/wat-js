...
as ordinary procedural macros for metaprogramming without
...
expressions will be executed with the variable bound to the exception.
...
simply returns the result of evaluating the handler body.
...
the result of the handler is returned.
