...
*** =let <lhs> . <rhs> → result= [operative]

Scheme-like =let= with two cases:

 * =(let <bindings> . <body>)= binds bindings and executes body.

 * =(let <loop> <bindings> . <body>)= executes body with a loop
   function.
