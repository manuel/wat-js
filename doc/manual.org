...
*** =names <env> → list= [applicative]

Returns a list of the symbols bound in the environment.  Does not
include symbols bound in ancestor environments.

*** =bound? <env> <sym> → boolean= [applicative]
...
Creates a new hashtable with =idhash= as hash function and =eq?= as
equality function.
...
*** =define-generic (<name> . <args>) → generic=

Defines a new generic function.

*** =define-method (<name> (<self> <type>) . <args>) . <body> → method=
