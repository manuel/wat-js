...
In addition to applicative and operative combiners, Wat also has
macros as an optimization mechanism.  Very common forms that are never
redefined, such as =let=, shouldn't suffer from the interpretetive
overhead of operatives.  Macros are an extra-linguistic mechanism and
should thus be used with care and only if performance is paramount.
