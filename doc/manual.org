...
/Virtua/ is a Lisp for web systems scripting.  Syntax is based on
...
asymmetric coroutines, providing cooperative multitasking.  Data uses
a dynamic type-tagging system that allows generic programming over
built-in as well as user-defined data types.

Virtua is hosted at <https://github.com/manuel/wat-js>.
...
Virtua has a mostly Scheme-like lexical structure, incompletely described
...
Virtua identifiers are case-sensitive.
 ...
 * String :: Virtua currently only supports UTF-16 encoded Unicode
   strings.

 * Number :: Virtua currently only supports IEEE 754 floating-point
   numbers.

 * Vector :: An array of objects.

 * Hashtable :: Virtua currently only supports identity hashtables.
...
All computations in Virtua are carried out by combiners, of which there
...
to somehow integrate it with Virtua's.  It is urged to convert JavaScript
objects to Virtua objects as early as possible with =from-js=, and
convert Virtua objects to JavaScript objects as late as possible with
=to-js=.  JavaScript objects may not implement all Virtua object
...
Tries to convert a Virtua object, such as a string, to a similar
JavaScript object.
