...
exception by invoking a restart.

** =handle <handlers> . <body> → result= [operative]

=handle= establishes exception handlers for the duration of the
evaluation of the body forms.  When one of the handlers matches a
thrown exception, it will be called on the stack - i.e. the stack is
not unwound.

Each handler is of the form =(<type-or-pred> (<var>)
. <handler-body>)=, where =<type-or-pred>= may either be a type or
predicate.  Handlers are matched against a thrown exception from left
to right.  If a thrown exception is of the given type or matches the
given predicate, then the =<handler-body>= expressions will be
executed with the variable =<var>= bound to the exception.

A handler may handle the exception by performing a nonlocal transfer
of control.  If the handler normally returns, the search for a handler
will continue up the stack.  If no handler is found, the
=default-handler= generic function is called on the exception.
...
** =catch <handlers> . <body> → result= [operative]
...
** =default-handler <exc> → result= [generic]

Called on an exception when no handler is found, or when all found
handlers returned normally.

** =throw <exc> . <handlers> → result= [operative]

Throws an exception and optionally provides restarts that an exception
handler may use to continue computation.  Restarts are installed as if
by =catch=, i.e. when a restart is invoked, the stack is unwound and
the result of the handler is returned.  =throw= only returns normally
if one of its restarts is invoked, otherwise it always enters the
debugger or crashes the program.

** =invoke-restart <restart> → result= [operative]

** =compute-restarts <exception> → list= [operative]

** =associated-exception <restart> → exc= [generic]

Returns the associated exception of a restart.  Associated exceptions
are a mechanism for ensuring that 
...
;;   (make-use-value-restart associated-exception value)
;;   #ign
...
;;        (Div-by-Zero-Exception (dbz) (throw (make-use-value-restart dbz 3))))
;;      4))
#+END_EXAMPLE
