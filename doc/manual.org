...
restart handlers will be effective as per =compute-restarts=.  This is
unlike the restart handlers bound by a =throw=, which are associated
with the thrown exception.  Restart handlers bound by =restart-bind=
...
** =compute-restarts <exception> → list= [applicative] /NIY/

Returns a list of restart handlers that are effective for the given
exception.  Restart handlers bound by =restart-bind= are effective for
any exception, whereas restart handlers bound by =throw= are only
